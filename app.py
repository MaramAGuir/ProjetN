...
csv_file_path = "https://github.com/nevermind78/DATALM/raw/main/1LM.csv"# url feha les note il kol
# Chargement du fichier CSV en nettoyant les espaces dans la colonne Email
df = pd.read_csv(csv_file_path, delimiter=";", converters={"Email": lambda x: x.strip()})#strip tne7i les espace mil 9odem wil teli,, trim tne7 mil wost
df = df[df['GROUP']=='1LM3']
...
df["Note"] = pd.to_numeric(df["Note"], errors='coerce').fillna(0)# filnna : il a w il absent iweliiw 0
